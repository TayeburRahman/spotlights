...
const CatagoriesLength = ({category}) =>{ 
    ...
    const categoryTools =  data && data.filter(
      (tool) => tool.category === category.replace(/-/g, " ")
    ); 
 return(
    <p className="text-1xl font-bold dark:text-gray-900 text-white text-left">{categoryTools?.length}</p>
 )
                ...
                {categories.map((category, index) => (
                    <Link to={`/ai-tools/${category?.replace(/\s+/g, "-")}`} key={index} value={category} className="m-3 max-w-sm p-6 dark:bg-slate-50 border dark:border-gray-200 rounded-[0.8rem]  dark:hover:bg-gray-100 bg-gray-800 border-gray-700 hover:bg-gray-700 flex justify-between items-center shadow-lg dark:bg-gradient-to-t dark:from-[#17dcff] bg-gradient-to-t from-[#17dcff]">
                        <div className='flex items-center gap-3'>
                           ...
                           <div>
                           <h5 className="mb-1 text-2xl font-bold tracking-tight dark:text-gray-900 text-white text-left text-up">{category}</h5>
                            <CatagoriesLength category={`${category?.replace(/\s+/g, "-")}`} />
                           </div>
