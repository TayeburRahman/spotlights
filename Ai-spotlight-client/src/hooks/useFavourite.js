...
const useFavourite = (item, user) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (user) {
          const response = await axios.get(`https://spotlight-server-1.onrender.com/api/v1/tools/featured/${item._id}/${user.email}`);
  ...
  const handelOnAddFeature = (id) => {
    ...
    axios.put(`https://spotlight-server-1.onrender.com/api/v1/tools/featured/${id}/${user?.email}`)
      .then(res => {
